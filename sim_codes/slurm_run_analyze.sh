...
cp $code_home/input_files/$ftype $sim_home

# cp $code_home/$finit ./

python_venv="#!/bin/bash -l
source ~/venv/containers/openmm/bin/activate
python3 run_sims.py -name $name -dt $dt -ftype $ftype -ftop $top  -fseq $seq -rep $replica -Ta $Ta -Na $Na -F $F -temp $T -kb $kb -Esoft $Esoft -nblocks $nblocks -blocksize $blocksize -R0 $R0 -finit $finit -kr $kr

python3 run_analyze.py -s $save_dest/analysis/ -gyr -RDP -MSD -bondlen -HiC
"
